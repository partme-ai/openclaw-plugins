...
│   ├── new-plugin.mjs         # Plugin scaffolding from _template
│   └── sync-message-sdk-deps.mjs
├── testing/                   # Standard test runner & test datasets
...
- Rule engine: match by channel / direction / topic / accountId, with `forward` and `reply-via` actions
- Template variable expansion (`{{channel}}`, `{{topic}}`, etc.) for dynamic routing targets
- `RouteDedupeCache` (TTL + capacity limit) prevents duplicate routing on webhook retries
- Audit logging to MQ without modifying channel code — see [router](./extensions/router)

#### 2. RAG Knowledge (knowledge)

- 5 embedding providers (OpenAI, DashScope, Zhipu, Qianfan, Ollama)
- 3 reranker providers (Ollama, Jina, Zhipu)
- 3 vector stores (sqlite-vec, zvec, zvec-native)
- Hybrid retrieval (vector similarity + keyword search), reranker, intent gate
