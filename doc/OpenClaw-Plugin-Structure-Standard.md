...
Base Profile 在 `src/` 下预置语义目录占位（`.gitkeep`），供 Extended Profile 渐进填充：
...
空目录 MUST 以 `.gitkeep` 保留；MAY 在首次实现时替换为模块文件。
...
| `test/e2e/` | MAY | 端到端测试占位 |
| `hooks/` | MAY | OpenClaw 生命周期钩子；空目录用 `.gitkeep` |
| `skills/` | MAY | Agent Skill 子目录 + `SKILL.md`；见 `skills/README.md` |
...
| Extended 语义目录缺失 | warn | warn | error（`wecom-kf`、`wecom`） | §7.2 |
...
| `src/*/.gitkeep` | §5.2 Extended 语义目录占位，首次实现时替换为模块 |
| `hooks/`、`skills/`、`test/e2e/` | §9 可选资产目录 |
