...
- Create a WeCom smart robot in API mode.
...
| Type | Limit | Fallback |
|------|-------|----------|
| Image | 10 MB | Send as file when possible. |
| Video | 10 MB | Send as file when possible. |
| Voice | 2 MB AMR | Non-AMR or oversized voice is treated as file. |
| File | `media.maxBytes` | Rejected when too large. |
...
- Copy Corp ID, Corp Secret, and Agent ID.
- Enable API message receiving and copy Token and EncodingAESKey.
...
- Install and configure the knowledge plugin separately.
...
| `network.timeoutMs` | Timeout for selected HTTP paths. |
