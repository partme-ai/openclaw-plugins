...
| 校验基线 | `scripts/check-plugin-structure.mjs` 按 §1.2 Profile 分流：Channel 插件校验 Base/Extended；Capability/Infra/SDK 校验根 manifest + 入口，跳过 Channel 平铺 MUST |

### 1.2 校验 Profile（Checker Profile）

结构校验按 **Profile** 分流，而非对所有插件套用 Channel Base 平铺规则。`scripts/check-plugin-structure.mjs` 读取 `openclaw.plugin.json`（`kind`、`channels`）并结合显式映射/heuristic 判定 Profile。
...
| **utility-minimal** | `router` | MUST NOT | 根或 `src/` 入口 | 路由/编排工具；不要求 Channel 资产 |

**Profile 判定优先级**：

1. 显式映射表（checker 内 `PLUGIN_PROFILE_OVERRIDE`）
2. Manifest：`kind: memory` → capability-memory；`kind: channel` → channel-base
3. Heuristic：`message-sdk` → sdk；`channels[]` 非空 → channel-base；`channels: []` 或缺失 → capability
...
至少提供一张与当前源码一致的 Mermaid 图；涉及 ACK、重试、授权、回调等时序时 SHOULD 再提供
`sequenceDiagram` 或 `stateDiagram-v2`。推荐最小表达如下：
...
文档更新 MUST 保留仍有效的配置样例、报文样例和故障处理说明。大量缩减文档前 SHOULD 在变更
说明中列出删除项及其失效依据；没有失效依据的原理图、代码样例和运维说明不得删除。
...
| 模式 | 命令 | 行为 |
|------|------|------|
| default | `node scripts/check-plugin-structure.mjs` | 按 Profile 分流；Channel 漂移以 warn 为主；`_template` / Tier A 的 Base MUST 缺失为 error |
| strict-base | `--strict` / `--strict-base` | 所有 **channel-base** 插件 Base Profile MUST 违规导致 exit 1 |
| strict-new | `--strict-new` | **channel-extended**（`wecom-kf`、`wecom`）Extended 阈值违规导致 exit 1 |
| CI 报告 | `--json` | 输出 JSON（含每插件 `profile`、`profileSource`），供流水线与报表集成 |

### 10.1.1 Profile 映射（显式）
...
| 检查项 | default | strict-base | strict-new | 适用 Profile |
|--------|---------|-------------|------------|--------------|
| Base 根目录 MUST 文件缺失 | warn（`_template`/Tier A：error） | error（channel-base） | warn | channel-* |
| Base `src/` Channel 平铺 MUST 缺失 | warn（`_template`/Tier A：error） | error（channel-base） | warn | channel-* |
| Capability 根目录（`.gitignore`、`LICENSE`） | warn | warn | warn | capability-*、infra、sdk-rag |
| SDK 最小集（`.gitignore`、`src/`） | warn | warn | warn | sdk |
| 插件入口（`src/index.ts` 或 manifest `main`） | warn | warn | warn | capability-*、infra、sdk-rag、utility-minimal |
| `openclaw.extensions[]` 缺失 | warn（channel：error） | error（channel-base） | warn | channel-*、capability-*、infra |
| `openclaw.setupEntry` 缺失 | warn（channel：error） | error（channel-base） | warn | channel-* only |
