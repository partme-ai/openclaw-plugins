...
export const ALL_CAPABILITIES: Record<string, ChannelCapabilities> = {
  ...
  "openclaw-weixin": {
    ...
    media: { inbound: MEDIA_FULL, outbound: MEDIA_FULL, maxFileSizeBytes: 20 * 1024 * 1024 },
    textLimits: { maxPerMessage: 20_000, overflowStrategy: "split" },
    threading: { supported: false, firstClass: false },
    reactions: { supported: false },
    groupChat: { supported: false, requireMention: false },
    escaping: { markdownDialect: "none" },
    quirks: ["iLink long-poll transport", "direct chat only", "context token required for replies", "block streaming"],
  },
  "wechat-ipad": {
    channelId: "wechat-ipad",
    supportedFormats: ["text"],
    ...
    groupChat: { supported: true, requireMention: false },
    escaping: { markdownDialect: "none" },
    quirks: ["unofficial external protocol bridge", "explicit risk acknowledgement required", "text-only in current implementation"],
  },
  "wecom-kf": {
    channelId: "wecom-kf",
    supportedFormats: ["text"],
    ...
    groupChat: { supported: true, requireMention: false },
    escaping: { markdownDialect: "none" },
    quirks: ["encrypted callback", "sync_msg cursor", "customer-service session state", "block streaming"],
  },
  douyin: {
    channelId: "douyin",
    supportedFormats: ["text"],
    ...
    groupChat: { supported: false, requireMention: false },
    escaping: { markdownDialect: "none" },
    quirks: ["life-service webhook", "no symmetric direct-message outbound protocol", "business operations use allowlisted OpenAPI tools"],
  },
  mqtt: {
    channelId: "mqtt",
    supportedFormats: ["text"],
    media: { inbound: MEDIA_NONE, outbound: MEDIA_NONE, maxFileSizeBytes: 0 },
    textLimits: { maxPerMessage: 1_048_576, overflowStrategy: "error" },
    threading: { supported: false, firstClass: false },
    reactions: { supported: false },
    groupChat: { supported: false, requireMention: false },
    escaping: { markdownDialect: "none" },
    quirks: ["topic-bound routing", "QoS is transport delivery only", "Bridge audit topics are excluded from recursive mirroring"],
  },
  discord: {
    channelId: "discord",
    supportedFormats: ["text", "markdown"],
