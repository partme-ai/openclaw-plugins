...
提供显式登记的小红书商家 API Tool。组件类型：**业务能力 Tool**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-rednode` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `rednode` |
| Channel ID | — |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/rednode` |
...
[Agent 小红书 Ark 操作]
          ...
          ▼
┌──────────────────────────────────────────────────────────────┐
│ OpenClaw Gateway 内: rednode
│ 1. 校验操作白名单、确认和速率
│ 2. 签名并调用 Ark Open API
│ 3. 限制响应并脱敏 Tool Result
└──────────────────────────────────────────────────────────────┘
          │
          ▼
[受控 Ark API 结果]
...
| 能力 | 说明 |
|---|---|
| 负责 | 提供显式登记的小红书商家 API Tool |
| 不负责 | 不是内容抓取器、消息 Channel 或审核替代 |
| 输入 | Agent 小红书 Ark 操作 |
| 输出 | 受控 Ark API 结果 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.rednode.config` |
| Channel 配置 | 不适用 |
| 配置 Schema | `extensions/rednode/openclaw.plugin.json` |
