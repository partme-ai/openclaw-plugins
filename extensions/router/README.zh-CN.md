...
提供可恢复、可观测的跨渠道消息转发。组件类型：**跨渠道可靠路由**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-router` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `router` |
| Channel ID | — |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/router` |
...
| 能力 | 说明 |
|---|---|
| 负责 | 提供可恢复、可观测的跨渠道消息转发 |
| 不负责 | 不改变目标 Channel 的鉴权和发送策略 |
| 输入 | OpenClaw 消息事件 |
| 输出 | 目标 Channel 消息与路由状态 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.router.config` |
| Channel 配置 | 不适用 |
| 配置 Schema | `extensions/router/openclaw.plugin.json` |
