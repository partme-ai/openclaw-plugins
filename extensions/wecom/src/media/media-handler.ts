 ...
 *
 * 企业微信 **Bot 长连接** 入站媒体下载与落盘（图片/文件）。
 *
 * **职责**：
 * - 通过 WSClient.downloadFile（或 fetchRemoteMedia 降级）拉取媒体
 * - 插件层主动校验 `resolveWecomMediaMaxBytes`，超限抛 `MediaOversizeError`
 * - 调用 OpenClaw Core `saveMediaBuffer` 写入 inbound 媒体目录
 *
 * **与 Agent 模式区别**：Agent 入站媒体在 `agent/handler` + `api-client.downloadMedia` 处理。
 */
...
import { IMAGE_DOWNLOAD_TIMEOUT_MS, FILE_DOWNLOAD_TIMEOUT_MS } from "../types/const.js";
import { withTimeout } from "../shared/timeout.js";
import { resolveWecomMediaMaxBytes, type ResolvedWeComAccount } from "../config/wecom-config.js";
