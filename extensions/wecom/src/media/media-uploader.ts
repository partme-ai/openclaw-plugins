 ...
 *
 * 企业微信 **Bot 出站** 媒体上传与发送（WSClient 路径）。
 *
 * **职责**：
 * - 从 mediaUrl 加载 buffer（远程/本地，见 `resolveMediaFile`）
 * - MIME → 企微媒体类型映射与大小降级策略
 * - `uploadAndSendMedia`：aibot_send_msg 主动发送
 * - `uploadAndReplyMedia`：aibot_respond_msg 被动回复（可覆盖 THINKING）
 *
 * **大小限制**：图片/视频 10MB、语音 2MB、绝对上限 20MB（见 `const.ts`）。
 */
...
import {
  ...
  detectMime,
  type WebMediaResult,
} from "../shared/openclaw-compat.js";
import {
  IMAGE_MAX_BYTES,
  VIDEO_MAX_BYTES,
  VOICE_MAX_BYTES,
