/**
 * @module webhook/helpers
 *
 * Webhook **辅助函数**集合（入站解析、stream 响应构建、兜底、本机路径、配置）。
 *
 * **职责**：
 * - `buildInboundBody` / `processInboundMessage`：入站文本与媒体解密
 * - `buildStreamResponse` / `buildStreamReplyFromState`：企微 stream 协议 JSON
 * - 本机路径提取、Agent dispatch config、taskKey 等
 *
 * **与 message-sdk 关系**：
 * - `truncateUtf8Bytes`（util）限制 stream/dm 字节
 * - 媒体解密委托 `webhook/media`；Path Guard 见 `media-path-guard`
 *
 * **关键导出**：`processInboundMessage`、`buildInboundBody`、`buildStreamResponse`、
 * `buildCfgForDispatch`、`buildFallbackPrompt`
 */
...
import { registerResponseUrl } from "../outbound/outbound-reply.js";
...
export async function processInboundMessage(
  ...
  const { resolveWecomEgressProxyUrl } = await import("../config/wecom-config.js");
