 ...
 * 企业微信 WS 入站消息内容解析。
 *
 * **职责**：从 SDK `WsFrame.body` 提取文本、图片/文件 URL（含 aeskey）、引用内容，
 * 并将模板卡片 / 文档权限变更等 **event 回调** 格式化为可路由给 Agent 的纯文本。
 *
 * **适用场景**：`monitor.prepareWeComMessage` Step 1；Webhook 入站链路复用同一解析器。
 *
 * **上下游**：
 * - 上游：`@wecom/aibot-node-sdk` WsFrame.body
 * - 下游：`media-handler`（URL 下载）、OpenClaw dispatch（textParts）
 *
 * **关键导出**：`parseMessageContent`、`MessageBody`、`ParsedMessageContent`
 */
...
/** SDK WebSocket 消息体（Bot 回调 JSON 结构） */
...
/** `parseMessageContent` 返回值 */
export interface ParsedMessageContent {
  /** 正文片段（多段时用换行拼接） */
  textParts: string[];
  /** 待下载图片 URL 列表 */
  imageUrls: string[];
  /** 图片 URL → aeskey 映射（解密必需） */
 ...
 *
 * Agent 可直接从 `question_key` / `option_id` 理解用户选择。
 *
 * @param body - WS 消息体
 * @returns 格式化文本；非 template_card_event 时返回 `undefined`
 */
...
/** 文档权限类型枚举（auth_change_event.auth_list 元素） */
enum AuthType {
  /** 新建和编辑文档 */
 ...
 *
 * 管理员在授权页变更权限后，系统推送 `auth_change_event`；
 * 根据 `auth_list` 生成操作指引，引导 Agent 继续或提示补授权。
 *
 * @param body - WS 消息体
...
/**
 * 解析消息内容（单条 / 图文混排 / 事件回调 / 引用消息）。
 *
 * **msgtype 分支**：
 * - `event`：模板卡片、文档权限变更 → 纯文本，无媒体
 * - `mixed`：遍历 msg_item 收集 text + image
 * - 其他：text / voice / image / file / video
 * - `quote`：补充引用文本或将引用媒体 URL 并入下载列表
 *
 * @param body - SDK WebSocket 消息体
 * @returns 文本片段、媒体 URL/aeskey、引用内容
 */
