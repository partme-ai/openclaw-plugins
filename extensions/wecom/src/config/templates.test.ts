...
describe("resolveWecomTemplates", () => {
  ...
  it("merges flat thinkingText overrides", () => {
  ...
  it("maps toolStatusText to internal tool template", () => {
    const resolved = resolveWecomTemplates({
      toolStatusText: "调用 {toolName}",
    } as WeComConfig);
    expect(resolveWecomToolStatusLine(resolved, "search")).toBe("调用 search");
  });
});
...
describe("resolveWecomToolStatusLine", () => {
  ...
  it("falls back to static tool template without placeholder", () => {
    expect(resolveWecomToolStatusLine(WECOM_DEFAULT_TEMPLATES)).toBe(
...
describe("buildMediaErrorSummary", () => {
  it("uses reason template for rejectReason", () => {
    const templates = resolveWecomTemplates({
      mediaErrorReasonText: "失败：{reason}",
    } as WeComConfig);
