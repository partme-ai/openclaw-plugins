/**
 * openclaw plugin-sdk 高版本 API 兼容层（openclaw-compat）
 *
 * 在较低版本 openclaw 上为 wecom 提供与新版 SDK 一致的媒体/MIME/allowFrom/状态目录等行为。
 * 模块加载时探测 SDK 导出，有则委托，无则本地 fallback（参考历史 weclaw 实现）。
 *
 * 与 message-sdk：
 * - `resolveStateDir` re-export 自 message-sdk/openclaw（见 state-dir-resolve.ts）
 * - 其余 re-export（readRequestBodyWithLimit、createPersistentDedupe 等）来自 runtime-api → message-sdk
 *
 * 其他 wecom 模块应从此文件或 runtime-api 导入，避免散落 import openclaw/plugin-sdk 版本分支。
 */
...
import { readGuardedLocalMediaFile } from "../media/media-path-guard.js";
...
async function loadOutboundMediaFromUrlFallback(
  ...
  // 本地文件：Path Guard + mediaLocalRoots 白名单
  ...
  if (!readResult.ok) {
    if (readResult.rejectReason === "not found") {
      throw new Error(`Local media file not found: ${mediaUrl}`);
    }
    throw new Error(readResult.error);
  }
  const data = readResult.buffer;
