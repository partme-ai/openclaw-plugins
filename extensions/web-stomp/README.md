...
Provides STOMP access for browsers and the Spring ecosystem. Component type: **STOMP/WebSocket wire channel**.

| Item | Value |
|---|---|
| npm package | `@partme.ai/openclaw-web-stomp` |
| Version | `2026.7.1` |
| Plugin ID | `web-stomp` |
| Channel ID | `stomp` |
| OpenClaw | `>=2026.7.1` |
| Source | `extensions/web-stomp` |

## 2. At a glance

```text
[Spring and web STOMP clients]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ Inside OpenClaw Gateway: web-stomp
│ 1. Upgrade WebSocket and negotiate STOMP
│ 2. Handle subscriptions, frames, heartbeats, and ACK
│ 3. Run the Agent and send STOMP MESSAGE frames
└──────────────────────────────────────────────────────────────┘
          │
          ▼
[Web STOMP messages and subscription state]
```
...
Spring and web STOMP clients
  │
  ▼
Upgrade WebSocket and negotiate STOMP
  │
  ▼
Handle subscriptions, frames, heartbeats, and ACK
  │
  ▼
Run the Agent and send STOMP MESSAGE frames
  │
  ▼
Web STOMP messages and subscription state
...
| Area | Contract |
|---|---|
| Owns | Provides STOMP access for browsers and the Spring ecosystem |
| Does not own | Does not implement SockJS, a full broker relay, or JMS |
| Input | Spring and web STOMP clients |
| Output | Web STOMP messages and subscription state |
...
| Layer | Path |
|---|---|
| Plugin configuration | `plugins.entries.web-stomp.config` |
| Channel configuration | `channels["stomp"]` |
| Configuration schema | `extensions/web-stomp/openclaw.plugin.json` |
