/**
 * FIFO outbound queue grouped by sessionKey.
 *
 * 出站队列用于把 Agent 产出的回复按会话分组缓存，适合需要异步 publish、
 * 批量 drain 或测试出站顺序的传输插件。它是内存结构，不负责持久化。
 */
 ...
 *
 * 不传 sessionKey 调用 `pop` 时会从最早发现的非空 session 列表取一条，
