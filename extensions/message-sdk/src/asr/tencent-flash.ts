...
export async function transcribeTencentFlash(params: {
  ...
  if (!Buffer.isBuffer(audio) || audio.length === 0) {
    throw new TypeError("Tencent Flash ASR audio must be a non-empty Buffer");
  }
  for (const [name, value] of Object.entries({
    ...
    if (typeof value !== "string" || !value.trim()) {
      // 缺失凭据属于配置错误，不应进入网络重试；沿用不可重试的 auth 分类便于调用方决策。
      throw new ASRAuthError(ASR_PROVIDER, `Tencent Flash ASR ${name} is required`);
    }
  }
  const timeoutMs = config.timeoutMs ?? 30000;
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
    throw new RangeError("Tencent Flash ASR timeoutMs must be a positive safe integer");
  }
