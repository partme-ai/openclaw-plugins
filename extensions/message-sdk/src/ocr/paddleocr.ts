...
import { resolveOcrTimeout, validateOcrBase64 } from "./validation.js";

const PROVIDER = "paddleocr";
const DEFAULT_MODEL = "PP-OCRv4";
const DEFAULT_BASE_URL = "http://localhost:8866/predict/ocr_system";

function resolveImageBase64(input: OCRInput, config: OCRConfig): string {
  if (input.base64) return validateOcrBase64(PROVIDER, input.base64, config);
...
export async function recognizePaddleOCR(input: OCRInput, config: OCRConfig): Promise<OCRResult> {
  ...
  try {
    const imageBase64 = resolveImageBase64(input, config);
