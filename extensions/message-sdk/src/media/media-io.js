...
export class FileSizeLimitError extends Error {
    actualSize;
    limitSize;
    constructor(message, actualSize, limitSize) {
        super(message);
...
export class PathSecurityError extends Error {
    unsafePath;
    reason;
    constructor(message, unsafePath, reason) {
        super(message);
...
function normalizeForCompare(value) { return path.resolve(value).replace(/\\/g, "/").toLowerCase(); }
...
export function validatePathSecurity(filePath, options = {}) {
    ...
    if (filePath.length > maxPathLength)
        throw new PathSecurityError(`Path length ${filePath.length} > ${maxPathLength}`, filePath, "path too long");
    if (preventTraversal && path.normalize(filePath).includes(".."))
        throw new PathSecurityError("Path traversal detected", filePath, "traversal");
    if (allowedPrefixes?.length) {
        const np = path.normalize(filePath);
        if (!allowedPrefixes.some((p) => np.startsWith(path.normalize(p))))
            throw new PathSecurityError("Path not in allowed prefixes", filePath, "not allowed");
    }
}
export function getDefaultAllowedPrefixes() {
...
// 媒体读取
// ============================================================================
export async function fetchMediaFromUrl(url, options = {}) {
    ...
    try {
        ...
        if (cl) {
            ...
            if (size > maxSize)
                throw new FileSizeLimitError(`Content-Length ${size} > ${maxSize}`, size, maxSize);
        }
        const arrayBuffer = await response.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        if (buffer.length > maxSize)
            throw new FileSizeLimitError(`Size ${buffer.length} > ${maxSize}`, buffer.length, maxSize);
        let fileName = "file";
        try {
        ...
        if (err instanceof Error && err.name === "AbortError")
            throw new MediaTimeoutError(`Timeout after ${timeout}ms`, timeout);
        throw err;
    }
    finally {
...
export async function downloadToTempFile(url, options = {}) {
    if (!isHttpUrl(url))
        throw new Error(`downloadToTempFile expects HTTP URL, got: ${url}`);
    ...
    try {
        ...
        if (cl) {
            ...
            if (!Number.isNaN(declared) && declared > maxSize)
                throw new FileSizeLimitError(`Content-Length ${declared} > ${maxSize}`, declared, maxSize);
        }
...
export async function readMediaFromLocal(filePath, options = {}) {
    ...
    if (stats.size > maxSize)
        throw new FileSizeLimitError(`File size ${stats.size} > ${maxSize}`, stats.size, maxSize);
    const buffer = await fsPromises.readFile(localPath);
    const fileName = path.basename(localPath);
    const mimeType = getMimeType(localPath);
    return { buffer, fileName, size: buffer.length, mimeType };
}
export async function readMedia(source, options = {}) {
    if (isHttpUrl(source))
        return fetchMediaFromUrl(source, options);
    return readMediaFromLocal(source, options);
}
export async function readMediaBatch(sources, options = {}) {
    const results = await Promise.allSettled(sources.map((s) => readMedia(s, options)));
    return results.map((r, i) => r.status === "fulfilled" ? { source: sources[i], result: r.value } : { source: sources[i], error: r.reason });
}
// ============================================================================
// 归档与清理
// ============================================================================
export async function finalizeInboundMediaFile(options) {
...
export async function pruneInboundMediaDir(options) {
    ...
    for (const entry of entries) {
        ...
        if (!dirStats.isDirectory())
            continue;
        if ((dirStats.mtimeMs || dirStats.ctimeMs || 0) >= cutoff)
            continue;
        let files = [];
        try {
...
export async function cleanupFileSafe(filePath, onError) {
    if (!filePath)
        return;
    try {
...
//# sourceMappingURL=media-io.js.map