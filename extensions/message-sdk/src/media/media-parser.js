...
// 路径工具
// ============================================================================
export function isHttpUrl(value) { return /^https?:\/\//i.test(value); }
export function isFileUrl(value) { return /^file:\/\//i.test(value); }
export function isLocalReference(raw) {
    if (isHttpUrl(raw))
        return false;
...
export function stripTitleFromUrl(value) {
...
export function isImagePath(filePath) { return IMAGE_EXTENSIONS.has(getExtension(filePath)); }
export function isNonImageFilePath(filePath) { return NON_IMAGE_EXTENSIONS.has(getExtension(filePath)); }
export function detectMediaTypeFromPath(filePath) {
...
function createExtractedMedia(source, sourceKind, options) {
    ...
    let fileName;
    if (isLocal) {
        localPath = normalizeLocalPath(clean);
        fileName = path.basename(localPath);
    }
    else if (isHttp) {
        try {
            fileName = path.basename(new URL(clean).pathname) || undefined;
        }
        catch { /* ignore */ }
    }
    return { source: clean, localPath, type: detectMediaTypeFromPath(fileName || clean), isLocal, isHttp, fileName, sourceKind };
}
export function extractMediaFromText(text, options = {}) {
    ...
    if (parseMediaLines) {
        ...
        for (const line of lines) {
            ...
            for (const c of payload.split(/\s+/).filter(Boolean)) {
                ...
                if (!cleaned || (!isHttpUrl(cleaned) && !isLocalReference(cleaned)))
                    continue;
...
//# sourceMappingURL=media-parser.js.map