...
迁移顺序（用户确认）：**Gotify → WeCom hook 化 → MQ 统一 ingress/stack → Feishu 高级 hooks**。传输层插件仅负责 connect / publish / subscribe；OpenClaw 前后逻辑下沉 message-sdk。WeCom 企业 API（`@wecom/aibot-node-sdk`、`response_url`、Agent API）保留在 wecom 插件。
...
| ID | 目标 |
|----|------|
| G1 | 建立 message-sdk 分层模块：`openclaw/`、`ingress/`、`dispatch/`、`reply/`、`lifecycle/`、`adapters/` |
...
| G5 | 8 个 MQ 插件统一 ingress + stack 接入模式，消除重复 dispatch 代码 |
...
| **入站** | `parseTransportPayload` → UnifiedMessage（可选） | 渠道事件 → `gotifyStreamToUnified` / webhook parse → UnifiedMessage |
...
- SDK 提供 `createTranscriptDispatch()` 与 `createWireDispatch()` 两个 facade，共享底层 `ingress/` + `reply/`。
...
> MQ 插件 **不** 迁移至 Transcript 路径；在 Phase 3 统一的是 **ingress 归一化 + stack + dedup + 文档化桥接**，而非替换 `dispatchInbound`。  
...
flowchart TB
  ...
  subgraph SDK["message-sdk"]
    ...
    subgraph Core["core/ + stack/ + dedup/"]
      UM[UnifiedMessage]
      Stack[Inbound/Outbound Stack]
      Dedup[Idempotency / Persistent Dedup]
    end
    subgraph Dispatch["dispatch/"]
      WireDisp[createWireDispatch]
      TxDisp[createTranscriptDispatch]
    end
    ...
    subgraph Adapters["adapters/"]
      GotifyAd[gotify]
      WeComAd[wecom deliver hooks]
      FeishuAd[feishu hooks]
    end
  ...
  Stack --> WireDisp --> DispReply
  Stack --> TxDisp --> RunAsm --> BufDisp
...
| **`dispatch/`** | `createWireDispatch`（wrap `dispatchInbound`）、`createTranscriptDispatch`（runAssembled 编排） | 新增；`bridge/`  re-export 保持兼容 |
...
| **`adapters/`** | 渠道薄层：`gotify.ts`（已有）、`wecom/`、`feishu/` deliver 扩展点 | 新增 wecom/feishu 子目录 |
| **`core/`** | UnifiedMessage, Envelope | 保持 |
| **`stack/`** | Inbound/Outbound stack | 保持；Phase 3 MQ 启用 |
...
- [ ] `dispatch/` 导出 `createWireDispatch`（thin wrapper，行为等同现有 bridge）
...
- Gotify 入站：`ingress/normalize` 包装现有 `gotifyStreamToUnified`
- 出站：继续使用 REST 人类文本；接入 `reply/createReplyDispatcherBundle`（若适用）
- Dedup：已用 `createIdempotencyCache` — 文档化为 Phase 1 完成项
- `createTranscriptDispatch` 封装现有 `turn.runAssembled` + fallback 逻辑（自 `gotify/src/channel.ts` 抽取）
...
- 引入 `adapters/wecom/deliver-stream.ts` 作为 SDK 回调注入点
...
- 可选启用 `InboundMessageStack` / `OutboundMessageStack`（配置开关，默认 off 保兼容）
- **不** 切换至 Transcript；继续 `createWireDispatch` → `dispatchInbound`
...
- 对齐 inventory 表中 🔶 / 📝 项至 SDK 或 adapters/feishu
...
|-----------|------|------|--------------|------|
| 文件头注释 + imports | 1–45 | 混合 | `adapters/wecom/types` | 类型留插件；通用 import 走 SDK |
...
| `template_card` JSON 检测与 `response_url` POST | 103–151 | **Plugin** | `adapters/wecom/template-card.ts` | 企微协议；群聊降级文案 |
...
| Bot window timeout + `buildFallbackPrompt` | 200–224 | **Plugin** | `adapters/wecom/bot-window.ts` | `BOT_WINDOW_MS` 企微策略 |
...
| 媒体错误 fallback prompt | 308–350 | **Plugin** | `adapters/wecom/fallback-prompts.ts` | 企微 UX 文案 |
