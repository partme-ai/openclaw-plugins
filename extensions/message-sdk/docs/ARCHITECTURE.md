...
| **SDK 入口** | `createWireDispatch` → `dispatchInbound` | `createTranscriptDispatch` → `turn.runAssembled` |
...
```mermaid
flowchart LR
  MQ[MQ plugins] --> WireDisp[createWireDispatch]
  IM[IM plugins] --> TxDisp[createTranscriptDispatch]
...
- **`createWireDispatch`**：thin wrapper，行为等同 `dispatchInbound`（MQ reply-pipeline 模式）
- **`createEmbeddedAgentDispatch`**：当前进程 `runEmbeddedAgent` → `serializeForTransport` → `deliver`
- **`createSubagentDispatch`**：`subagent.run` + `waitForRun` → `serializeForTransport` → `deliver`
- **`createChannelDispatch`**：按 `mode` 路由上述三种 Wire 实现（MQ 插件统一入口）
...
|------|----------|---------------|----------|
| `reply-pipeline` | `createWireDispatch` | `dispatchInbound` + reply pipeline | mqtt, redis-stream, stomp |
| `embedded-agent` | `createEmbeddedAgentDispatch` | `agent.runEmbeddedAgent` | rabbitmq, rocketmq（默认） |
| `subagent` | `createSubagentDispatch` | `subagent.run` + `waitForRun` | rabbitmq, rocketmq |

MQ 类插件标准路径应使用 **`createChannelDispatch`**（或按 mode 直接调用子 facade），避免各插件复制 dispatch 逻辑。

## Gotify（Transcript reference）

Gotify 入站仍使用渠道专用的 `finalizeInboundContext`（Body/SessionKey 等字段），但：

- 入站归一化：`ingress/normalizeGotifyIngress` → `UnifiedMessage`
- 派发：`createTranscriptDispatch`（`turn.runAssembled` + fallback record）
...
├── dispatch/          # createWireDispatch, createTranscriptDispatch, createChannelDispatch
...
├── stack/             # InboundMessageStack, OutboundMessageStack
├── dedup/             # createIdempotencyCache, persistent dedupe
├── bridge/            # dispatchInbound, createReplyHandler (legacy + re-export)
├── adapters/          # gotify 等渠道薄适配
└── media|http|asr|... # 横切工具
```
...
1. 入站：`normalizeWireIngress` → `createChannelDispatch({ mode, reply: { deliver } })`
...
2. 派发：`createTranscriptDispatch`（保证 Control UI transcript）
3. 出站：渠道 REST / webhook deliver（人类可读）
4. **禁止** 引入 `dispatchInbound`（会丢失 Control UI 用户轮次）
