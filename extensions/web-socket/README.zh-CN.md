...
提供轻量嵌入式 WebSocket 消息入口。组件类型：**原生 WebSocket Channel**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-web-socket` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `web-socket` |
| Channel ID | `web-socket` |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/web-socket` |

## 2. 一眼看懂

```text
[WebSocket JSON/Text 客户端]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ OpenClaw Gateway 内: web-socket
│ 1. 鉴权、连接限额与协议解析
│ 2. 映射会话并进入 Agent
│ 3. 按连接路由发送响应
└──────────────────────────────────────────────────────────────┘
...
| 能力 | 说明 |
|---|---|
| 负责 | 提供轻量嵌入式 WebSocket 消息入口 |
| 不负责 | 不提供跨节点连接共享或通用 Socket.IO 协议 |
| 输入 | WebSocket JSON/Text 客户端 |
| 输出 | WebSocket 消息与连接状态 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.web-socket.config` |
| Channel 配置 | `channels["web-socket"]` |
| 配置 Schema | `extensions/web-socket/openclaw.plugin.json` |
