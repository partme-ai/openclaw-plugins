...
export const WECHAT_IPAD_CONFIG_JSON_SCHEMA = {
  ...
  properties: {
    ...
    message: {
      ...
      additionalProperties: false,
      properties: {
        dmPolicy: { type: "string", enum: ["allowlist", "open", "disabled"], default: "allowlist" },
        allowFrom: { type: "array", maxItems: 1000, items: { type: "string", minLength: 1, maxLength: 256 }, default: [] },
        commandAllowFrom: { type: "array", maxItems: 1000, items: { type: "string", minLength: 1, maxLength: 256 }, default: [] },
        handleGroup: { type: "boolean", default: false },
        groupWhitelist: { type: "array", maxItems: 1000, items: { type: "string", minLength: 1, maxLength: 256 }, default: [] },
        ...
        maxPendingMessages: { type: "integer", minimum: 1, maximum: 10_000, default: 256 },
      },
    },
  },
} as const;
...
function booleanValue(value: unknown, fallback: boolean, name: string): boolean {
  ...
  if (typeof value !== "boolean") throw new Error(`wechat-ipad: ${name} must be a boolean`);
  return value;
}
...
function identifierArray(value: unknown, name: string): string[] {
  ...
  if (!Array.isArray(value) || value.length > 1000 || value.some((entry) =>
    typeof entry !== "string" || !entry.trim() || entry.trim().length > 256 || /[\s/?#\u0000-\u001f\u007f]/u.test(entry.trim()))) {
    throw new Error(`wechat-ipad: ${name} must contain at most 1000 valid wxids`);
  }
  return [...new Set(value.map((entry) => entry.trim()))];
}

function dmPolicyValue(value: unknown): WechatIpadConfig["message"]["dmPolicy"] {
  if (value === undefined) return DEFAULT_CONFIG.message.dmPolicy;
  if (value !== "allowlist" && value !== "open" && value !== "disabled") {
    throw new Error("wechat-ipad: message.dmPolicy must be allowlist, open, or disabled");
  }
  return value;
}
...
function tokenValue(value: unknown, name: string): string {
  if (value === undefined) return "";
  if (typeof value !== "string") throw new Error(`wechat-ipad: ${name} must be a string`);
  const token = value.trim();
  if (!token || token.length > 8192 || /[\u0000-\u001f\u007f]/u.test(token)) {
    throw new Error(`wechat-ipad: ${name} must be a non-empty token without control characters`);
  }
  return token;
}
...
function endpointHostKey(value: string): string {
  ...
  return LOCAL_HOSTS.has(url.hostname) ? "loopback" : url.hostname.toLowerCase();
}
...
export function resolveWechatIpadConfig(
  ...
  assertAllowedKeys(raw, ["enabled", "acknowledgeUnofficialProtocolRisk", "required", "allowSplitBridgeHosts", "serviceUrl", "apiUrl", "reconnect", "auth", "network", "message"], "config");
  ...
  assertAllowedKeys(message, ["dmPolicy", "allowFrom", "commandAllowFrom", "handleGroup", "groupWhitelist", "allowAllGroups", "ignoreSelf", "maxTextChars", "maxPendingMessages"], "message");
  ...
  const commandAllowFrom = identifierArray(message.commandAllowFrom, "message.commandAllowFrom");
  if (enabled && dmPolicy === "allowlist" && allowFrom.length === 0) {
    throw new Error("wechat-ipad: message.dmPolicy=allowlist requires a non-empty message.allowFrom");
  }
  const handleGroup = booleanValue(message.handleGroup, DEFAULT_CONFIG.message.handleGroup, "message.handleGroup");
  const allowAllGroups = booleanValue(
    message.allowAllGroups,
    DEFAULT_CONFIG.message.allowAllGroups,
    "message.allowAllGroups",
  );
  const groupWhitelist = identifierArray(message.groupWhitelist, "message.groupWhitelist");
  ...
  const envToken = tokenValue(env.WECHAT_IPAD_BRIDGE_TOKEN, "WECHAT_IPAD_BRIDGE_TOKEN");
  const token = configuredToken || envToken || undefined;
  const serviceUrl = validateEndpoint(raw.serviceUrl, DEFAULT_CONFIG.serviceUrl, "websocket");
  ...
  if (enabled && (endpointIsRemote(serviceUrl) || endpointIsRemote(apiUrl)) && !token) {
    throw new Error("wechat-ipad: remote bridge endpoints require auth.token");
  }
  if (enabled && endpointHostKey(serviceUrl) !== endpointHostKey(apiUrl) && !allowSplitBridgeHosts) {
    throw new Error(
  ...
  return {
    ...
    apiUrl,
    reconnect: {
      enabled: booleanValue(reconnect.enabled, DEFAULT_CONFIG.reconnect.enabled, "reconnect.enabled"),
    ...
    message: {
      ...
      groupWhitelist,
      allowAllGroups,
      ignoreSelf: booleanValue(message.ignoreSelf, DEFAULT_CONFIG.message.ignoreSelf, "message.ignoreSelf"),
      maxTextChars: boundedInteger(message.maxTextChars, DEFAULT_CONFIG.message.maxTextChars, 1, 100_000, "message.maxTextChars"),
      maxPendingMessages: boundedInteger(message.maxPendingMessages, DEFAULT_CONFIG.message.maxPendingMessages, 1, 10_000, "message.maxPendingMessages"),
    },
  };
}
