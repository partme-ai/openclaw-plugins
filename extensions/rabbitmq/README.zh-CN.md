...
提供 RabbitMQ Topic 路由与可靠消费。组件类型：**AMQP Wire Channel**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-rabbitmq` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `rabbitmq` |
| Channel ID | `rabbitmq` |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/rabbitmq` |
...
| 能力 | 说明 |
|---|---|
| 负责 | 提供 RabbitMQ Topic 路由与可靠消费 |
| 不负责 | 不替代 Broker 集群、DLX 和权限运维 |
| 输入 | RabbitMQ Exchange / Queue |
| 输出 | AMQP 消息与 Agent 回复 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.rabbitmq.config` |
| Channel 配置 | `channels["rabbitmq"]` |
| 配置 Schema | `extensions/rabbitmq/openclaw.plugin.json` |
