...
Provides MQTT channel access for web clients. Component type: **MQTT/WebSocket wire channel**.

| Item | Value |
|---|---|
| npm package | `@partme.ai/openclaw-web-mqtt` |
| Version | `2026.7.1` |
| Plugin ID | `web-mqtt` |
| Channel ID | `mqtt-ws` |
| OpenClaw | `>=2026.7.1` |
| Source | `extensions/web-mqtt` |

## 2. At a glance

```text
[Browser MQTT over WebSocket traffic]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ Inside OpenClaw Gateway: web-mqtt
│ 1. Establish WebSocket and MQTT sessions
│ 2. Apply topic policy, decoding, and deduplication
│ 3. Run the Agent and publish bound replies
└──────────────────────────────────────────────────────────────┘
          │
          ▼
[Web MQTT messages]
...
Browser MQTT over WebSocket traffic
  │
  ▼
Establish WebSocket and MQTT sessions
  │
  ▼
Apply topic policy, decoding, and deduplication
  │
  ▼
Run the Agent and publish bound replies
  │
  ▼
Web MQTT messages
...
| Area | Contract |
|---|---|
| Owns | Provides MQTT channel access for web clients |
| Does not own | Does not replace production MQTT broker clustering |
| Input | Browser MQTT over WebSocket traffic |
| Output | Web MQTT messages |
...
```bash
openclaw plugins install "@partme.ai/openclaw-web-mqtt@2026.7.1"
```
...
| Layer | Path |
|---|---|
| Plugin configuration | `plugins.entries.web-mqtt.config` |
| Channel configuration | `channels["mqtt-ws"]` |
| Configuration schema | `extensions/web-mqtt/openclaw.plugin.json` |
...
```bash
pnpm --filter "@partme.ai/openclaw-web-mqtt" typecheck
pnpm --filter "@partme.ai/openclaw-web-mqtt" test
pnpm --filter "@partme.ai/openclaw-web-mqtt" build
```
