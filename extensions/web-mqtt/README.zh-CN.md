...
为 Web 客户端提供 MQTT Channel 接入。组件类型：**MQTT/WebSocket Wire Channel**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-web-mqtt` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `web-mqtt` |
| Channel ID | `mqtt-ws` |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/web-mqtt` |

## 2. 一眼看懂

```text
[浏览器 MQTT over WebSocket]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ OpenClaw Gateway 内: web-mqtt
│ 1. 建立 WebSocket 与 MQTT 会话
│ 2. 应用 Topic 策略、解码和去重
│ 3. 运行 Agent 并发布绑定回复
└──────────────────────────────────────────────────────────────┘
          │
...
| 能力 | 说明 |
|---|---|
| 负责 | 为 Web 客户端提供 MQTT Channel 接入 |
| 不负责 | 不替代生产 MQTT Broker 的集群能力 |
| 输入 | 浏览器 MQTT over WebSocket |
| 输出 | Web MQTT 消息 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.web-mqtt.config` |
| Channel 配置 | `channels["mqtt-ws"]` |
| 配置 Schema | `extensions/web-mqtt/openclaw.plugin.json` |
