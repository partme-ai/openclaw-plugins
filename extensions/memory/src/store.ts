/**
 * @fileoverview 本地分层长期记忆的持久化、隔离、加密和搜索实现。
 *
 * 数据按 Agent、会话、记忆层级和日期写入 JSONL；会话目录使用带本地隔离密钥的不可逆
 * 标识，避免原始 sessionKey 泄露。可选 AES-256-GCM 对每行独立加密，并通过校验标记阻止
 * 错误密钥静默读取。写操作按目标文件串行排队，初始化还负责旧目录迁移、保留期清理及
 * 活动文件发现。
 */
import {
  createCipheriv,
