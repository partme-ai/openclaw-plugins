  ...
  const execute = (handler: (params: Record<string, unknown>) => Promise<Record<string, unknown>>) =>
    async (_toolCallId: string, params: unknown): Promise<ToolResult> => {
      ...
      try {
        ...
        return result({ success: false, error: safeErrorMessage(error, [config.key, config.privateKey]) });
      }
    };
...
function safeErrorMessage(error: unknown, configuredSecrets: Array<string | undefined>): string {
