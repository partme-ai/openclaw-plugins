...
- 包、清单、插件 ID、依赖及安装元数据统一对齐 OpenClaw 2026.7.1。
- iLink 更新游标仅在整批消息处理成功后提交，失败重启按至少一次语义恢复，不再跳过未处理消息。
- 持久化有界的 7 天消息完成 ID，批次重试和进程重启时抑制已完成消息重放。
- 限制 API 与媒体响应大小，配置/下载端点强制 HTTPS，限制长轮询超时，并从日志移除敏感正文与身份标识。
- Channel 配置改为严格 Schema，并支持 section/account 级端点配置。
