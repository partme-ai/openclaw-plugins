...
describe("Session Management", () => {
  it("upsertSessionContext binds peer to OpenClaw sessionKey", () => {
    const key = "agent:agent-1:direct:device-001";
    upsertSessionContext(key, {
      peerId: "device-001",
      agentId: "agent-1",
      accountId: "default",
    });
    expect(getPeerIdBySession(key)).toBe("device-001");
    upsertSessionContext(key, {
      peerId: "device-001",
      agentId: "agent-1",
      accountId: "default",
  ...
  it("removePeerSessions cleans up all maps", () => {
    const key = "agent:agent-x:direct:cleanup-peer";
...
function upsertSessionContext(key: string, ctx: Record<string, unknown>): void {
  const peerId = (typeof ctx.peerId === "string" ? ctx.peerId : undefined) ?? _sessionPeerMap.get(key) ?? "";
  if (peerId) {
    _peerSessionMap.set(peerId, key);
    _sessionPeerMap.set(key, peerId);
  }
