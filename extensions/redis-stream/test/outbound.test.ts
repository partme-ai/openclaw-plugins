...
describe("redisStreamOutbound.sendText", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(publishMessage).mockResolvedValue(1);
  });
  ...
  it("uses XADD in stream channelMode", async () => {
    ...
    expect(publishEntry).toHaveBeenCalledWith("openclaw:outbound", {
      text: "agent reply",
    });
  ...
  it("rejects Pub/Sub delivery when Redis reports no active subscriber", async () => {
    vi.mocked(publishMessage).mockResolvedValueOnce(0);

    await expect(
      redisStreamOutbound.sendText({
        cfg: {
          channels: {
            "redis-stream": {
              url: "redis://localhost:6379",
    ...
    ).rejects.toThrow("No active subscriber");
  });
  ...
  it("uses the Router target and propagates its idempotency key in stream mode", async () => {
    ...
    expect(publishEntry).toHaveBeenCalledWith("audit:events", {
      body: "routed",
      idempotencyKey: "router-id",
    });
  });
  ...
  it("does not treat a normal durable outbound id as a Router direct target", async () => {
    ...
    expect(publishEntry).toHaveBeenCalledWith("safe-default", {
      body: "core reply",
    });
  });
});
