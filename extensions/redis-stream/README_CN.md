...
- **dmScope 会话隔离** -- 会话键完全基于 OpenClaw 全局 `session.dmScope` 配置生成
...
Channel 模式支持 `*` 通配符（glob 风格，以冒号分隔）。`*` 是贪婪匹配，匹配剩余所有分段。
...
| `defaultAgentId` | string | `""` | 无路由匹配时的兜底 Agent ID，空字符串=丢弃 |
| `subscribeChannels` | string[] | `[]` | 频道白名单，空数组=接受全部 |
| `channelBindings[].channelPattern` | string | -- | 频道模式（支持 `*` 通配符） |
...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `fieldMapping.textField` | `"text"` | 消息文本对应的键名 |
| `fieldMapping.agentIdField` | `"agentId"` | 目标 Agent 对应的键名 |
| `fieldMapping.peerIdField` | `"peerId"` | Peer 标识对应的键名 |
| `fieldMapping.accountIdField` | `"accountId"` | 账户上下文对应的键名 |
| `fieldMapping.replyStreamField` | `"replyStream"` | 回复 Stream 名称对应的键名 |

### Stream 设置

| 字段 | 默认值 | 说明 |
|------|--------|------|
| `stream.inboundKey` | `"openclaw:inbound"` | 消费组读取的 stream 键 |
| `stream.outboundKey` | `"openclaw:outbound"` | 回复写入的 stream 键 |
| `stream.consumerGroup` | `"openclaw-group"` | 消费者组名称 |
| `stream.consumerName` | `"openclaw-consumer-1"` | 消费者名称 |
| `stream.blockMs` | `5000` | XREADGROUP 阻塞超时（ms） |
| `stream.count` | `10` | 每批次最大消息数 |
| `stream.createGroup` | `true` | 自动创建消费者组 |

### 连接与负载设置

| 字段 | 默认值 | 说明 |
|------|--------|------|
| `payload.mode` | `"jsonTextOrPlain"` | 解析模式：`plain` 或 `jsonTextOrPlain` |
| `connection.reconnectMs` | `3000` | 重连延迟（ms） |
| `connection.maxRetries` | `10` | 最大重连次数 |

### 环境变量

| 变量 | 说明 |
|------|------|
| `REDIS_URL` | Redis 连接 URL（覆盖配置中的 `url`） |

## 生命周期

1. **Gateway 启动** -> 加载插件，注册 `redis-stream` 渠道
2. **账户启动** -> 连接 Redis，订阅频道（Pub/Sub）或创建消费组（Stream）
3. **消息接收** -> 入站消息经白名单过滤 -> 路由解析 -> dmScope 会话映射 -> Agent 分发
4. **Agent 回复** -> 通过 `PUBLISH`（Pub/Sub）或 `XADD`（Stream）发送回复
5. **Gateway 关闭** -> 取消订阅，退出 Redis 连接

## 消息处理流程

1. 接收 Redis channel 消息
2. 白名单检查：如果 `subscribeChannels` 非空，仅处理匹配的 channel
3. 路由解析：先查 `channelBindings`（显式匹配），回退到标准格式
...
8. 通过 Redis 将回复发送到 replyChannel

## 常见问题

**Pub/Sub 还是 Stream？**

Pub/Sub 用于实时、即发即忘的消息传递（类似聊天）。Stream 用于需要消费组、消息持久化和回放能力的场景（类似事件溯源）。

**会话隔离如何工作？**

会话键完全由 OpenClaw 全局 `session.dmScope` 配置生成。设置 `per-peer` 实现每个设备的隔离，或设为 `per-account-channel-peer` 实现完整的多租户。

**`*` 通配符的匹配规则是什么？**

`*` 是**贪婪**匹配，会匹配频道名中剩余的所有级别。例如 `openclaw:*` 匹配 `openclaw:a`、`openclaw:a:b`、`openclaw:a:b:c`。这与 Redis PSUBSCRIBE 的 `*` 仅匹配单个片段的行为不同。

**是否支持 Redis Cluster？**

Pub/Sub 可跨 Redis Cluster 节点工作。Stream 消费组在集群模式下需要谨慎的键路由，推荐使用单实例 Redis。

## 安全

- Redis 凭据通过连接 URL 提供：`redis://user:pass@host:port`
...
基于 Redis 团队的 [node-redis](https://github.com/redis/node-redis) 构建。会话隔离模式与 OpenClaw 的 openclaw-mqtt、openclaw-stomp 和 openclaw-rabbitmq 插件对齐。
