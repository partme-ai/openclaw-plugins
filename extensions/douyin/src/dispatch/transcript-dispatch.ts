...
import { resolveDouyinAgentReplyTimeoutMs } from "../config/resolvers.js";
import { deliverDouyinAgentReplyPayload } from "./outbound-reply.js";
...
function createLogger(opts: { log?: (message: string) => void; error?: (message: string) => void }) {
  return {
    info: (message: string) => (opts.log ?? console.log)(`[douyin] ${message}`),
    warn: (message: string) => (opts.log ?? console.log)(`[douyin] [WARN] ${message}`),
    error: (message: string) => (opts.error ?? console.error)(`[douyin] [ERROR] ${message}`),
  };
}

/**
 * 解析抖音入站 transcript 路由与会话存储路径。
 */
export function resolveDouyinTranscriptRoute(params: {
  runtime: PluginRuntime;
  cfg: Record<string, unknown>;
  accountId: string;
  peerId: string;
  log?: (message: string) => void;
}): { route: DouyinTranscriptRoute; storePath?: string } | null {
 ...
 * 构建抖音 Webhook 入站上下文。
 */
export function buildDouyinTranscriptInboundContext(params: {
  runtime: PluginRuntime;
  cfg: Record<string, unknown>;
  accountId: string;
  peerId: string;
  ...
  const fromLabel = `user:${peerId}`;
  const from = `${CHANNEL_ID}:user:${peerId}`;
  const to = `user:${peerId}`;
...
/**
 * 经 message-sdk `dispatchTranscriptTurn` 派发抖音 Webhook 入站。
 */
export async function dispatchDouyinTranscriptTurn(
  params: DouyinTranscriptDispatchParams,
): Promise<DouyinTranscriptDispatchResult | null> {
  ...
  const inboundContext = buildDouyinTranscriptInboundContext({
    runtime,
    cfg,
    accountId,
    peerId,
