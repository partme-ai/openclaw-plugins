...
Provides an embedded broker, topic bindings, and a reliable inbound queue. Component type: **MQTT wire channel**.

| Item | Value |
|---|---|
| npm package | `@partme.ai/openclaw-mqtt` |
| Version | `2026.7.1` |
| Plugin ID | `mqtt` |
| Channel ID | `mqtt` |
| OpenClaw | `>=2026.7.1` |
| Source | `extensions/mqtt` |

## 2. At a glance

```text
[MQTT client publish and subscribe traffic]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ Inside OpenClaw Gateway: mqtt
│ 1. Broker authentication, topic policy, and limits
│ 2. Decode, deduplicate, and enter the Agent
│ 3. Encode replies and publish to bound topics
└──────────────────────────────────────────────────────────────┘
          │
          ▼
[MQTT messages and session results]
```
...
MQTT client publish and subscribe traffic
  │
  ▼
Broker authentication, topic policy, and limits
  │
  ▼
Decode, deduplicate, and enter the Agent
  │
  ▼
Encode replies and publish to bound topics
  │
  ▼
MQTT messages and session results
...
| Area | Contract |
|---|---|
| Owns | Provides an embedded broker, topic bindings, and a reliable inbound queue |
| Does not own | Does not replace external MQTT cluster governance or device PKI |
| Input | MQTT client publish and subscribe traffic |
| Output | MQTT messages and session results |
...
| Layer | Path |
|---|---|
| Plugin configuration | `plugins.entries.mqtt.config` |
| Channel configuration | `channels["mqtt"]` |
| Configuration schema | `extensions/mqtt/openclaw.plugin.json` |
