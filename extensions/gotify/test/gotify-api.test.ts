...
import {
  mapGotifyToInbound,
  mapOutboundToGotify,
} from "../src/dispatch/routing/message-mapper.js";
...
describe("Message API", () => {
  ...
  it("builds message request with title and priority", () => {
    const req = buildMessageRequest(account, {
      message: "test",
      title: "Alert",
      priority: 8,
    });
  ...
  it("sends message and returns response", async () => {
    const fetchImpl = mockFetch([
      { ok: true, json: async () => ({ id: 123, message: "hello" }) },
    ]);
  ...
  it("does not blindly retry non-idempotent message POST on network failure", async () => {
    const fetchImpl = vi
      .fn()
    ...
    ).rejects.toThrow("Gotify connection failed");
    expect(fetchImpl).toHaveBeenCalledOnce();
  });
  ...
  it("does not blindly retry non-idempotent message POST on 5xx", async () => {
    ...
    ).rejects.toThrow("Gotify API failed (503)");
    expect(fetchImpl).toHaveBeenCalledOnce();
  });

  it("allows an operator to explicitly opt into unsafe POST retries", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce({
        ok: false,
  ...
  it("throws when missing appToken", () => {
    ...
    expect(() => buildMessageRequest(noToken, { message: "x" })).toThrow(
      "not configured",
    );
  });
    ...
    const fetchImpl = mockFetch([
      {
        ...
        json: async () => ({
          messages: [],
          paging: { size: 0, limit: 50, next: null, since: 10 },
        }),
      },
    ]);
    ...
    const fetchImpl = mockFetch([
      {
        ...
        json: async () => ({
          messages: [],
          paging: { size: 0, limit: 200, next: null, since: 0 },
        }),
      },
    ]);
  ...
  it("getApplicationMessages by app id", async () => {
    ...
    const result = await getApplicationMessages(account, 7, undefined, {
      fetchImpl,
    });
    ...
    const fetchImpl = mockFetch([
      {
        ok: true,
        json: async () => [
          { id: 1, name: "App1", token: "A...", internal: false },
        ],
      },
    ]);
...
describe("dispatchInboundMessage agent fallback", () => {
  it('resolves agent to "main" when resolveAgentRoute returns no agentId', () => {
    function resolveAgent(route: { agentId?: string | null } | null): string {
      return typeof route?.agentId === "string" && route.agentId.trim()
        ? route.agentId
        : "main";
    }
...
describe("outbound", () => {
  it("prefers explicit accountId", () => {
    expect(selectAccountId({ cfg: {}, accountId: "ops", to: "default" })).toBe(
      "ops",
    );
  });
  ...
  it("falls back to target when accountId is empty", () => {
    expect(
      selectAccountId({ cfg: {}, accountId: "", to: "gotify:alerts" }),
    ).toBe("alerts");
  });
  ...
  it("trims accountId whitespace", () => {
    expect(
      selectAccountId({ cfg: {}, accountId: "  ops  ", to: "default" }),
    ).toBe("ops");
  });
});
...
describe("multi-account config", () => {
  ...
  it("disables inbound when no clientToken", () => {
    const cfgNoClient = {
      channels: {
        gotify: {
          accounts: { basic: { serverUrl: "https://x.com", appToken: "x" } },
        },
      },
    };
