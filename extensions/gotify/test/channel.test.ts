...
const listenerMocks = vi.hoisted(() => ({
  ...
  stop: vi.fn(),
  deps: null as null | {
    onMessage: (message: GotifyStreamEnvelope) => Promise<void>;
  },
  replay: vi.fn(),
}));

vi.mock("../src/transport/ws-listener.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../src/transport/ws-listener.js")>();
  return {
    ...actual,
    createGotifyWsListener: (...args: unknown[]) =>
      listenerMocks.create(...args),
  };
});

vi.mock("../src/dispatch/backlog-replay.js", () => ({
  replayBacklogForAccount: (...args: unknown[]) =>
    listenerMocks.replay(...args),
}));
...
import { runInboundDispatchWithRetry } from "../src/channel/channel.js";
...
vi.mock("../src/transport/gotify-api.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../src/transport/gotify-api.js")>();
  return {
    ...actual,
    deleteMessage: vi.fn().mockResolvedValue(undefined),
    sendGotifyMessageWithDeliveryRetry: vi
      .fn()
      .mockResolvedValue({ id: 999, appid: 1 }),
...
describe("dispatchInboundMessage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    listenerMocks.start.mockResolvedValue(undefined);
    listenerMocks.stop.mockImplementation(() => undefined);
    listenerMocks.replay.mockResolvedValue({
      replayed: 0,
      lastSeenMessageId: 0,
    });
    listenerMocks.create.mockImplementation((_account, deps) => {
      listenerMocks.deps = deps;
      return { start: listenerMocks.start, stop: listenerMocks.stop };
    });
    resetGotifyRuntimeForTest();
    cleanupGotifyChannel();
  });
  ...
  it("uses resolveGotifyPeerId for routing peer id", async () => {
    const resolveAgentRoute = vi
      .fn()
      .mockResolvedValue({ agentId: "main", sessionKey: "sk" });
  ...
  it("does not mark dedup when dispatch fails so the message can be retried", async () => {
    ...
    await expect(
      dispatchInboundMessage(ctx as never, account, message),
  ...
  it("maps inbound text to Body fields for agent prompt", async () => {
    ...
    const dispatch = vi.fn().mockResolvedValue(undefined);
    const ctx = makeCtx({ dispatchReplyWithBufferedBlockDispatcher: dispatch });
    (ctx.channelRuntime.reply.finalizeInboundContext as ReturnType<
      typeof vi.fn
  ...
  it("resolves ConversationLabel and SenderName from Gotify application API", async () => {
    ...
    (ctx.channelRuntime.reply.finalizeInboundContext as ReturnType<
      typeof vi.fn
    >) = finalizeInboundContext;
    (resolveApplicationName as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      "Alert Manager",
    );
    ...
    expect(resolveApplicationName).toHaveBeenCalledWith(
  ...
  it("continues dispatch when optional application-name lookup fails", async () => {
    vi.mocked(resolveApplicationName).mockRejectedValueOnce(
      new Error("metadata API unavailable"),
    );
    const dispatch = vi.fn().mockResolvedValue(undefined);
    const ctx = makeCtx({ dispatchReplyWithBufferedBlockDispatcher: dispatch });
  ...
  it("does not use dispatchInbound from message-sdk bridge (transcript path only)", async () => {
    const bridge = await import("@partme.ai/openclaw-message-sdk/bridge");
    const dispatchInboundSpy = vi.spyOn(bridge, "dispatchInbound");
    const ctx = makeCtx();

    await dispatchInboundMessage(ctx as never, makeAccount(), {
      id: 501,
      appid: 5,
      message: "no wire path",
    });

    expect(dispatchInboundSpy).not.toHaveBeenCalled();
    dispatchInboundSpy.mockRestore();
  });
});

describe("runInboundDispatchWithRetry", () => {
  it("retries transient dispatch failures with bounded backoff", async () => {
    vi.useFakeTimers();
    const task = vi
      .fn()
      .mockRejectedValueOnce(new Error("temporary"))
      .mockResolvedValueOnce(undefined);
    const pending = runInboundDispatchWithRetry({
      task,
      maxAttempts: 3,
      initialDelayMs: 100,
      maxDelayMs: 1000,
  ...
  it("fails after the configured number of dispatch attempts", async () => {
    vi.useFakeTimers();
    const task = vi.fn().mockRejectedValue(new Error("poison"));
    const pending = runInboundDispatchWithRetry({
      task,
      maxAttempts: 2,
      initialDelayMs: 100,
    ...
    const rejection = expect(pending).rejects.toThrow("exhausted 2 attempts");
...
describe("Gotify gateway fail-closed lifecycle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    listenerMocks.start.mockResolvedValue(undefined);
    listenerMocks.stop.mockImplementation(() => undefined);
  ...
  it("stops the listener after live dispatch retry exhaustion", async () => {
    const account = resolveGotifyAccount(
      {
        channels: {
          gotify: {
    ...
    const dispatch = vi.fn().mockRejectedValue(new Error("agent unavailable"));
    const ctx = makeCtx({ dispatchReplyWithBufferedBlockDispatcher: dispatch });
    ctx.account = account;
    const startPromise = gotifyChannel.gateway!.startAccount(ctx as never);

    await vi.waitFor(() => expect(listenerMocks.replay).toHaveBeenCalledOnce());
    await new Promise((resolve) => setImmediate(resolve));
    await expect(
      listenerMocks.deps!.onMessage({
        id: 900,
        appid: 42,
        message: "retry then stop",
      }),
    ).rejects.toThrow("exhausted 1 attempts");
    ...
    expect(ctx.setStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        running: false,
        lastError: expect.stringContaining("exhausted 1 attempts"),
      }),
    );
  });
});
