...
function resolveAgentConfigOrThrow(params: {
  ...
  if (requestedAccountId) {
    if (!resolvedAccounts.accounts[requestedAccountId]) {
      throw new Error(
        `WeCom outbound account "${requestedAccountId}" not found. Configure channels.wecom-cs.accounts.${requestedAccountId} or use an existing accountId.`,
      );
    }
  }
  ...
  if (!account?.configured) {
    throw new Error(
      `WeCom outbound requires Agent mode for account=${params.accountId ?? "default"}. Configure channels.wecom-cs.accounts.<accountId>.agent (or legacy channels.wecom-cs.agent).`,
    );
  }
  if (typeof account.agentId !== "number" || !Number.isFinite(account.agentId)) {
    throw new Error(
      `WeCom outbound requires channels.wecom-cs.accounts.<accountId>.agent.agentId (or legacy channels.wecom-cs.agent.agentId) for account=${params.accountId ?? account.accountId}.`,
    );
  }
