...
vi.mock("../media/path-guard.js", async (importOriginal) => {
    ...
    return {
        ...actual,
        getExtendedMediaLocalRoots: async () => [os.tmpdir()],
        readGuardedLocalMediaFile: async ({ filePath }: { filePath: string }) => ({
            ok: true as const,
...
function createMockRequest(bodyObj: any): IncomingMessage {
    ...
    req.method = "POST";
    req.url = "/plugins/wecom-kf/bot/default?timestamp=123&nonce=456&signature=789";
