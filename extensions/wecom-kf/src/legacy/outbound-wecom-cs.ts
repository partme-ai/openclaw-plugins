...
async function loadMediaBuffer(
  ...
  const wecomConfig = cfg?.channels?.["wecom-kf"] as WecomConfig | undefined;
  const allowedRoots = await getExtendedMediaLocalRoots(wecomConfig);
  const guarded = await readGuardedLocalMediaFile({ filePath: mediaUrl, allowedRoots });
  if (!guarded.ok) {
  ...
  return { buffer: guarded.buffer, contentType, filename };
}
...
function resolveAgentConfigOrThrow(params: {
  ...
  if (requestedAccountId) {
    if (!resolvedAccounts.accounts[requestedAccountId]) {
      throw new Error(
        `WeCom outbound account "${requestedAccountId}" not found. Configure channels.wecom-kf.accounts.${requestedAccountId} or use an existing accountId.`,
      );
    }
  }
  ...
  if (!account?.configured) {
    throw new Error(
      `WeCom outbound requires Agent mode for account=${params.accountId ?? "default"}. Configure channels.wecom-kf.accounts.<accountId>.agent (or legacy channels.wecom-kf.agent).`,
    );
  }
  if (typeof account.agentId !== "number" || !Number.isFinite(account.agentId)) {
    throw new Error(
      `WeCom outbound requires channels.wecom-kf.accounts.<accountId>.agent.agentId (or legacy channels.wecom-kf.agent.agentId) for account=${params.accountId ?? account.accountId}.`,
    );
  }
...
export const wecomLegacyOutbound: ChannelOutboundAdapter = {
  deliveryMode: "direct",
  chunkerMode: "text",
  ...
  sendMedia: async ({ cfg, to, text, mediaUrl, accountId }: ChannelOutboundContext) => {
    ...
    if (botAccount?.connectionMode === "websocket" && botAccount.configured) {
      ...
      if (!rawTo.startsWith("wecom-kf-agent:")) {
        ...
        const { buffer, contentType, filename } = await loadMediaBuffer(mediaUrl, cfg);
