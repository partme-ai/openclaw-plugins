...
把 OpenMem 作为可插拔记忆后端接入。组件类型：**外部记忆桥**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-openmem` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `openmem` |
| Channel ID | — |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/openmem` |

## 2. 一眼看懂

```text
[OpenClaw 记忆查询与生命周期事件]
          │
          ▼
┌──────────────────────────────────────────────────────────────┐
│ OpenClaw Gateway 内: openmem
│ 1. 按租户和会话构造请求
│ 2. 调用 OpenMem HTTP 召回或写入
│ 3. 限制、清洗并合并结果
└──────────────────────────────────────────────────────────────┘
          │
          ▼
...
| 能力 | 说明 |
|---|---|
| 负责 | 把 OpenMem 作为可插拔记忆后端接入 |
| 不负责 | 不部署 OpenMem 服务，也不绕过其租户隔离 |
| 输入 | OpenClaw 记忆查询与生命周期事件 |
| 输出 | 外部记忆上下文与写入状态 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.openmem.config` |
| Channel 配置 | 不适用 |
| 配置 Schema | `extensions/openmem/openclaw.plugin.json` |
