...
const plugin = {
  id: "wecom-cs",
  name: "WeCom CS",
  description: "OpenClaw WeCom CS (WeChat Work) intelligent bot + ICS REST API plugin",
  ...
  register(api: OpenClawPluginApi) {
    ...
    const routes = ["/plugins/wecom-cs", "/wecom-cs"];
    ...
    // Register wecom_cs_mcp
    api.registerTool(createWeComMcpTool(), { name: "wecom_cs_mcp" });

    // MEDIA instruction prompt injection
    api.on("before_prompt_build", (_event, ctx) => {
      if (ctx.channelId !== "wecom-cs") return;
      return {
        systemPrompt: [
          "【发送文件/图片/视频/语音】",
