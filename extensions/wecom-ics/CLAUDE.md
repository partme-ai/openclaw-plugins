...
|------|---------|--------------|--------------|
| **Bot** (智能体) | Real-time streaming chat | `/wecom-cs`, `/wecom-cs/bot` | Streaming responses, low latency, text/image only |
| **Agent** (自建应用) | Fallback & broadcast | `/wecom-cs/agent` | File sending, broadcasts, long tasks (>6min) |
...
openclaw config set channels.wecom-cs.dynamicAgents.enabled true

# Configure admin users (use main agent)
openclaw config set channels.wecom-cs.dynamicAgents.adminUsers '["admin1","admin2"]'
```
...
- **Paths**: `/wecom-cs` (legacy), `/wecom-cs/bot`, `/wecom-cs/agent`
...
- **Max Size**: 25MB default (configurable via `channels.wecom-cs.media.maxBytes`)
