# OpenClaw 企业微信（WeCom CS）Channel 插件
...
```bash
openclaw plugins install @mocrane/wecom-cs
openclaw plugins enable wecom-cs
```
...
openclaw config set channels.wecom-cs.enabled true
openclaw config set channels.wecom-cs.defaultAccount ops

# 2) 新增 ops 账号（Bot + Agent）
openclaw config set channels.wecom-cs.accounts.ops '{
  "enabled": true,
  "name": "运维机器人",
  "bot": {
    "aibotid": "BOT_OPS",
...
openclaw config set channels.wecom-cs.accounts.sales '{
  "enabled": true,
  "name": "销售机器人",
  "bot": {
    "aibotid": "BOT_SALES",
...
openclaw config set bindings '[{"agentId":"ops-agent","match":{"channel":"wecom-cs","accountId":"ops"}},{"agentId":"sales-agent","match":{"channel":"wecom-cs","accountId":"sales"}}]'

# 5) 验证配置
openclaw config get channels.wecom-cs
openclaw channels status
```

Webhook 回调建议按账号分别配置：
- Bot（推荐）：`/plugins/wecom-cs/bot/{accountId}`
- Agent（推荐）：`/plugins/wecom-cs/agent/{accountId}`
...
```bash
openclaw config set channels.wecom-cs.network.egressProxyUrl "http://proxy.company.local:3128"
```
...
```jsonc
{
  "channels": {
    "wecom-cs": {
      ...
      "media": {
        "tempDir": "/tmp/openclaw-wecom-cs-media",
        "retentionHours": 24,
        "cleanupOnStart": true,
        "maxBytes": 26214400
      },
  ...
  "bindings": [
    { "agentId": "ops-agent", "match": { "channel": "wecom-cs", "accountId": "ops" } },
    { "agentId": "sales-agent", "match": { "channel": "wecom-cs", "accountId": "sales" } }
  ]
}
```
...
| 模式 | 路径 | 说明 |
|:---|:---|:---|
| Bot（推荐，多账号） | `/plugins/wecom-cs/bot/{accountId}` | 指定账号回调（例如 `/plugins/wecom-cs/bot/default`） |
| Agent（推荐，多账号） | `/plugins/wecom-cs/agent/{accountId}` | 指定账号回调（例如 `/plugins/wecom-cs/agent/default`） |

### 2.4 从单账号迁移到多账号（4 步）

1. 把原来的 `channels.wecom-cs.bot` / `channels.wecom-cs.agent` 拆到 `channels.wecom-cs.accounts.default.bot/agent`。
2. 按业务继续新增 `channels.wecom-cs.accounts.<accountId>`（例如 `ops`、`sales`）。
3. 为每个账号增加 `bindings[].match.accountId`，映射到对应 OpenClaw agent。
4. 企业微信后台把回调 URL 改成账号路径：`/plugins/wecom-cs/bot/{accountId}`、`/plugins/wecom-cs/agent/{accountId}`，然后执行 `openclaw channels status` 验证。
...
3. 创建机器人，选择 **API 模式**
4. 填写回调 URL：`https://your-domain.com/plugins/wecom-cs/bot/{accountId}`（例如默认账号：`https://your-domain.com/plugins/wecom-cs/bot/default`）
...
4. **重要：** 进入「企业可信IP」→「配置」→ 添加你服务器的 IP 地址
   - 如果你使用内网穿透/动态 IP，建议配置 `channels.wecom-cs.network.egressProxyUrl` 走固定出口代理，否则可能出现：`60020 not allow to access from your ip`
5. 在应用详情中设置「接收消息 - 设置API接收」
6. 填写回调 URL：`https://your-domain.com/plugins/wecom-cs/agent/{accountId}`（例如默认账号：`https://your-domain.com/plugins/wecom-cs/agent/default`）
...
- 【SDK适配】♻️ **插件 HTTP 注册升级**：入口改为 `registerHttpRoute`（`/plugins/wecom-cs` + `match=prefix` + `auth=plugin`），适配 OpenClaw 新版插件接口。
- 【兼容修复】🔁 **旧入口保持可达**：同步注册 `/wecom-cs` 前缀路由，保障历史 Bot/Agent 回调地址继续可用。
- 【兼容性修复】🧩 **OpenClaw 3.1 路由抢占问题修复**：推荐回调地址升级为 `/plugins/wecom-cs/bot/{accountId}`、`/plugins/wecom-cs/agent/{accountId}`，规避根路径 Control UI fallback 抢占 webhook。
- 【引导收敛】🧭 **Onboarding 仅支持账号化配置**：配置向导统一写入 `channels.wecom-cs.accounts.<accountId>`，不再引导单账号旧结构。
- 【兼容策略】🔁 **旧路径兼容保留**：`/wecom-cs/*` 历史回调路径保留兼容能力，但不再作为维护主路径。
...
- 【可控性】🛠 **账号管理修复**：`deleteAccount` 改为仅删除目标账号，不再误删整个 `channels.wecom-cs`。
- 【易用性】📘 **文档默认路径调整**：README 快速开始与配置说明改为优先展示“多账号 + 多 Agent”矩阵配置；单账号模式保留为兼容方案。
- 【质量保障】✅ **回归保护**：新增账号解析、冲突检测、动态路由隔离、生命周期与入站过滤测试。

### 2026.2.7

- ✨ **动态 Agent 路由**：新增按用户/群组隔离的动态路由功能，支持为每个对话主体自动分配独立的 Agent 实例（如 `wecom-cs-dm-xxx`）。
...
- ⚙️ **精细化配置**：新增 `channels.wecom-cs.dynamicAgents` 配置项，可独立控制私聊和群聊的动态路由行为。
- 🙏 **特别致谢**：感谢 [WhyMeta](https://github.com/WhyMeta) 开发者对动态路由实施方案的贡献。

### 2026.2.5

- 🛠 **体验优化**：WeCom 媒体（图片/语音/视频/文件）处理的默认大小上限提升到 25MB，减少大文件因超限导致的“下载/保存失败”。
- 📌 **可配置提示**：若仍遇到 Media exceeds ... limit，日志/回复会提示通过 channels.wecom-cs.media.maxBytes 调整上限，并给出可直接执行的 openclaw config set 示例命令。
