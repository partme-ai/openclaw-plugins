/**
 * 美团渠道 message-sdk 配置解析（媒体上限、Agent 超时、出口代理）。
 */
 ...
 * 解析美团入站/出站媒体最大字节数。
 */
export function resolveMeituanMediaMaxBytes(cfg: ChannelLimitsOpenClawConfig): number {
 ...
 * 解析 Agent 回复派发超时（毫秒）。
 */
export function resolveMeituanAgentReplyTimeoutMs(cfg: ChannelLimitsOpenClawConfig): number {
 ...
 * 解析 HTTP 出站代理 URL。
 */
export function resolveMeituanEgressProxyUrl(cfg: ChannelLimitsOpenClawConfig): string | undefined {
