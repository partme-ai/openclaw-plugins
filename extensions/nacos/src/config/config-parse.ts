 ...
 *
 * 将 `plugins.entries.nacos.config` 归一化为 {@link NacosPluginConfig}。
...
function parseConfigCenter(raw: unknown): NacosConfigCenterConfig | undefined {
  ...
  const sharedConfigs =
    parseSharedConfigs(raw.sharedConfigs) ??
    parseSharedConfigs(raw["shared-configs"]);
  ...
  const cc: NacosConfigCenterConfig = {
    ...
    ...(typeof raw.primaryConfigDataId === "string" &&
    raw.primaryConfigDataId.trim() !== ""
      ? { primaryConfigDataId: raw.primaryConfigDataId.trim() }
      : {}),
    ...(typeof raw.primaryConfigGroup === "string" &&
    raw.primaryConfigGroup.trim() !== ""
      ? { primaryConfigGroup: raw.primaryConfigGroup.trim() }
      : {}),
    ...(sharedConfigs ? { sharedConfigs } : {}),
    ...(typeof raw.applicationDataId === "string" &&
    raw.applicationDataId.trim() !== ""
    ...
    ...(typeof raw.backupRetentionCount === "number" &&
    Number.isInteger(raw.backupRetentionCount) &&
    raw.backupRetentionCount >= 1 &&
    raw.backupRetentionCount <= 1000
      ? { backupRetentionCount: raw.backupRetentionCount }
      : {}),
  };
...
export function parseNacosPluginConfig(raw: unknown): ParsePluginConfigResult {
  ...
  const serverList =
    typeof flat.serverList === "string" ? flat.serverList.trim() : "";
  if (!serverList) {
    return {
      kind: "skip",
  ...
  const namingServerList =
    typeof flat.namingServerList === "string" &&
    flat.namingServerList.trim() !== ""
      ? flat.namingServerList.trim()
      : undefined;
  const configServerList =
    typeof flat.configServerList === "string" &&
  ...
  const config: NacosPluginConfig = {
    ...
    startupFailurePolicy:
      flat.startupFailurePolicy === "degrade" ? "degrade" : "fail",
    serverList,
    ...
    ...(typeof flat.username === "string" && flat.username !== ""
      ...
      : {}),
    ...(typeof flat.password === "string" && flat.password !== ""
    ...
    ...(typeof flat.clusterName === "string" && flat.clusterName.trim() !== ""
      ...
      : {}),
    ...(typeof flat.weight === "number" && Number.isFinite(flat.weight)
      ? { weight: flat.weight }
      : {}),
    ...
    ...(namingRaw && namingRaw.enabled === false
