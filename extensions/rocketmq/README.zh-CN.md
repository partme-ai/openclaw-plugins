...
提供 Topic 绑定、消费重试和回复发布。组件类型：**RocketMQ Wire Channel**。

| 项目 | 内容 |
|---|---|
| npm 包 | `@partme.ai/openclaw-rocketmq` |
| 当前版本 | `2026.7.1` |
| 插件 ID | `rocketmq` |
| Channel ID | `rocketmq` |
| OpenClaw | `>=2026.7.1` |
| 源码目录 | `extensions/rocketmq` |
...
[RocketMQ Topic 消息]
          ...
          ▼
┌──────────────────────────────────────────────────────────────┐
│ OpenClaw Gateway 内: rocketmq
│ 1. 启动 Producer / PushConsumer
│ 2. 解码、去重并运行 Agent
│ 3. 发送回复并处理消费结果
└──────────────────────────────────────────────────────────────┘
          │
...
| 能力 | 说明 |
|---|---|
| 负责 | 提供 Topic 绑定、消费重试和回复发布 |
| 不负责 | 不替代 NameServer/Broker 集群治理 |
| 输入 | RocketMQ Topic 消息 |
| 输出 | RocketMQ 消息与消费状态 |
...
| 配置层 | 路径 |
|---|---|
| 插件配置 | `plugins.entries.rocketmq.config` |
| Channel 配置 | `channels["rocketmq"]` |
| 配置 Schema | `extensions/rocketmq/openclaw.plugin.json` |
