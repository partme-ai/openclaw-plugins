...
import { DEFAULT_ROCKERMQ_CONFIG, type RockermqConfig } from "../src/config.js";
...
describe("topic-router", () => {
  describe("parseStandardTopic", () => {
    ...
    it("should reject ambiguous multi-segment peer ids", () => {
  ...
  describe("buildReplyTopicFromInbound", () => {
    ...
    it("should append -out for a non-standard bound topic", () => {
  ...
  describe("buildOutboundTopic", () => {
    ...
    it("should reject reserved delimiters and broker-unsafe characters", () => {
      expect(() => buildOutboundTopic("agent--1", "openclaw")).toThrow("must not contain '--'");
      expect(() => buildOutboundTopic("agent.1", "openclaw")).toThrow("must use");
    });
  });
});
